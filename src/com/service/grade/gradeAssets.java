...
public class gradeAssets {
	private List<TGradeAssets> gradeAssetsList;
	private TGradeAssets item;
	...
	public List<TGradeAssets> getGradeAssetsList() {
		return gradeAssetsList;
	}

	public void setGradeAssetsList(List<TGradeAssets> gradeAssetsList) {
		this.gradeAssetsList = gradeAssetsList;
	}
	...
	public TGradeAssets getItem() {
		return item;
	}
	public void setItem(TGradeAssets item) {
		this.item = item;
	}
	public String returnList() throws Exception {
		gradeAssetsList=(OperateDao.getList("from TGradeAssets T where T.treeid=" +getTreeid(), null));
		double sum=0.0000;
		for(TGradeAssets i:gradeAssetsList)
		{
			sum+=i.getGradesum();
		}
		TGradeAssets sumTGradeAssets=new TGradeAssets();
		sumTGradeAssets.setReviewexpert("平均分");
		DecimalFormat df=new DecimalFormat("#.#");
		
		sumTGradeAssets.setGradesum(Double.parseDouble(df.format(sum/gradeAssetsList.size())));
		gradeAssetsList.add(sumTGradeAssets);
		return "list";
	}
	public String AddAssets(){
		item=new TGradeAssets();
	...
	public String SaveAssets() throws Exception 
	{
		Double sum=0.0;
	...
	public String AssetsAction() throws Exception {
		return "ok";
	}
    public String EditAssets()
    {
    	setItem((TGradeAssets) OperateDao.getObject("from TGradeAssets T where T.id=" +getId(), null));
    	return "ok";
    }	
    public String EditOpinion()
    {
    	TGradeAssets i=(TGradeAssets) OperateDao.getObject("from TGradeAssets T where T.id=" +getId(), null);
	...
	public String downOpinion() throws IOException
	{
		...
		Poi poi = new Poi();
		File file=poi.ExpotOpinion(root+"upload"+File.separator +fileName, "from TGradeAssets T where T.treeid=" +getTreeid(),"TGradeAssets");
    ...
    {
    	OperateDao.ExecuteBySql("delete TGradeAssets T where T.id=" +getId());
    	return "ok";
    }
}
